...
| `fairness` | `-(own_queue + α·own_wait + β·Σ neigh_queue + λ·max_red_time)` | Add pressure to reduce phase starvation (longest red-phase duration) |
...
- **Fairness** `max_red_time`: maximum value in the environment’s internal `_red_time` counter for the junction — the number of simulation steps each logical phase (horizontal / vertical) has been red — divided by **300.0**. This penalises phases that have been starved of green time longest, not per-lane `getWaitingTime`.
...
|---|---|---|
| 0–7 | Lane traffic | 4 approaches × (`queue/50`, `wait/300`); wait = `min(getWaitingTime, 300)` per lane |
...
| Discount γ | `0.99` |
| ε start / min / decay | `1.0` / `0.01` / `0.999988` (per environment step) |
...
- Each episode sets `env.seed` to **`seed + episode`** (deterministic SUMO traffic variation). Pass `--randomize_episode_seed` to use a random integer in `0–9999` per episode instead.
...
**Checkpoint naming:** `agent_<JUNCTION>_ep<N>.pth` and `agent_<JUNCTION>_final.pth` (e.g. `agent_J1_final.pth`).
...
**Model fails to load in evaluation**  
